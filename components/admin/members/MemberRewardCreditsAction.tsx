...
export default function MemberRewardCreditsAction({
  memberId,
  memberName,
  currentBalance,
  onSuccess,
  ...
  async function handleSubmit() {
    ...
    if (
      ...
      setError(
        "Enter an amount greater than RM0."
      );
      return;
    }
    ...
    if (
      ...
      setError(
        "The deduction cannot exceed the current Reward Credits balance."
      );
      return;
    }
  ...
  return (
    <>
      <button
        type="button"
        onClick={openDialog}
        className="flex h-11 items-center gap-3 rounded-xl border border-violet-400/15 bg-violet-400/5 px-4 text-left text-sm text-violet-300 transition hover:bg-violet-400/10"
      >
          ...
          <div className="relative max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-white/[0.09] bg-slate-900 p-6 shadow-2xl shadow-black/50 sm:p-7">
            ...
            <div className="mt-6">
              ...
              <div className="relative mt-2">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="h-14 w-full rounded-2xl border border-white/[0.08] bg-slate-950/55 pl-12 pr-4 text-lg font-semibold text-white outline-none focus:border-violet-400/40 focus:ring-4 focus:ring-violet-400/10"
                />
              </div>
            </div>
            ...
            <div className="mt-5">
              ...
              <textarea
                ...
                rows={4}
                placeholder="Explain why this Reward Credits adjustment is required"
                className="mt-2 w-full resize-none rounded-2xl border border-white/[0.08] bg-slate-950/55 px-4 py-3 text-sm text-white outline-none placeholder:text-slate-700 focus:border-violet-400/40 focus:ring-4 focus:ring-violet-400/10"
              />
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <button
                ...
                disabled={loading}
                className="h-11 rounded-xl border border-white/[0.08] px-5 text-sm font-medium text-slate-400 hover:bg-white/[0.06]"
              >
                Cancel
              </button>
